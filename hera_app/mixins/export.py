...
class ExportMixin:
    ...
    def _export_hypercube_envi_with_roi(self, hypercube_handle, output_base_path, description, info=None, log_label="hypercube"):
        ...
        if self._is_owned_cube_info(info):
            return self._export_owned_hypercube_envi_with_roi(
                hypercube_handle,
                output_base_path,
                description,
                info,
    ...
    def _export_owned_hypercube_envi_with_roi(self, hypercube_handle, output_base_path, description, info, log_label):
        try:
            import numpy as np
        except Exception:
            np = None

        source_width = int(info.get("source_width") or info.get("width") or 0)
        source_height = int(info.get("source_height") or info.get("height") or 0)
        bands = int(info.get("bands") or 0)
        data_type = int(info.get("data_type") if info.get("data_type") is not None else 0)
        if source_width <= 0 or source_height <= 0 or bands <= 0:
            raise RuntimeError(f"Cached {log_label} has invalid dimensions for export.")

        export_roi = info.get("export_roi") or info.get("display_roi")
        if export_roi:
            roi_x, roi_y, roi_w, roi_h = (int(value) for value in export_roi)
            ...
            roi_x, roi_y, roi_w, roi_h = 0, 0, source_width, source_height
        ...
        out_dtype = "<f4" if data_type == 0 else "<f8"
        ...
        try:
            self._log_async(
                f"Exporting cached {log_label}: x={roi_x}, y={roi_y}, "
                f"w={roi_w}, h={roi_h}, bands={bands}."
            )
            with open(temp_raw_path, "wb") as raw_file:
                for band_index in range(bands):
                    wavelength, values, band_width, _band_height = self._get_cube_band_flat_values(
                        hypercube_handle,
                        info,
                        band_index,
                    )
                    if np is not None:
                        band_array = np.asarray(values).reshape((source_height, source_width))
                        roi_array = band_array[roi_y:roi_y + roi_h, roi_x:roi_x + roi_w]
                        np.asarray(roi_array, dtype=out_dtype).tofile(raw_file)
                    else:
                        for row in range(roi_y, roi_y + roi_h):
                            start = row * band_width + roi_x
                            array.array(array_typecode, values[start:start + roi_w]).tofile(raw_file)
                    wavelengths.append(float(wavelength))
                    if band_index == 0 or (band_index + 1) % 25 == 0 or band_index + 1 == bands:
                        self._log_async(f"Cached export progress ({log_label}): band {band_index + 1}/{bands}")

            safe_description = (description or "Generated by Hyperspectral Scanning Acquisition APP").replace("}", ")")
            if export_roi:
                safe_description = (
                    f"{safe_description}\n"
                    f"Cached ROI export: x={roi_x}, y={roi_y}, width={roi_w}, height={roi_h}"
                )

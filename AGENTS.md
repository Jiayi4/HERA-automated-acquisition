...
- Flatfield follows the Hera Acquisition App manual: acquire a white diffuse/reference surface, keep that hypercube in `flatfield_hypercube_handle`, and for compatible later sample cubes export/display normalized data by dividing sample pixels by matching flatfield pixels. Compatibility currently requires matching source size, displayed ROI, band count, and data type.
